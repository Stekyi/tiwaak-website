export const NAV_LINKS = [
  { label: "About", href: "#about" },
  { label: "Services", href: "#services" },
  { label: "Why Us", href: "#why-us" },
  { label: "Contact", href: "#contact" },
]

export const CONTACT_EMAIL = "info@tiwaak.com"
export const FORMSPREE_ENDPOINT = "https://formspree.io/f/xqejyzaj"

export const SERVICES = [
  {
    title: "Trading",
    description:
      "We facilitate global commodity and goods trading with precision, connecting buyers and sellers across markets.",
    offerings: [
      "Commodity & goods trading",
      "Market analysis & intelligence",
      "Import/export facilitation",
      "Supply chain optimization",
    ],
    icon: "chart",
  },
  {
    title: "Business Consultancy",
    description:
      "We deliver strategic advisory services that help businesses grow, optimise operations, and enter new markets.",
    offerings: [
      "Market entry strategy",
      "Operational efficiency",
      "Financial advisory",
      "Business development",
    ],
    icon: "briefcase",
  },
]

export const WHY_US = [
  {
    title: "Proven Expertise",
    description: "Years of hands-on experience in trading and strategic consultancy across diverse industries.",
    icon: "star",
  },
  {
    title: "Global Reach",
    description: "Networks spanning multiple markets and continents, opening doors others cannot.",
    icon: "globe",
  },
  {
    title: "Trusted Partner",
    description: "Client relationships built on transparency, integrity, and consistent results.",
    icon: "shield",
  },
  {
    title: "Tailored Solutions",
    description: "Every strategy is designed specifically for your business context and goals.",
    icon: "target",
  },
]

export const CORE_VALUES = [
  {
    number: "01",
    title: "Integrity",
    description: "We operate with unwavering honesty and transparency in every engagement.",
  },
  {
    number: "02",
    title: "Precision",
    description: "Data-driven decisions and meticulous execution define our approach.",
  },
  {
    number: "03",
    title: "Partnership",
    description: "Your success is our success. We invest in long-term relationships.",
  },
]
